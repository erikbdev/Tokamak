...
extension UnsafeMutablePointer where Pointee == GtkWidget {
  ...
  func connect(
    signal: UnsafePointer<gchar>,
    closure: @escaping (UnsafeMutablePointer<GtkWidget>?, OpaquePointer) -> Void
  ) -> Int {
    let closureBox = Unmanaged.passRetained(DualParamClosureBox(closure)).retain().toOpaque()
    return connect(
      signal: signal,
      data: closureBox,
      handler: { widget, context, closureBox in
        let unpackedAction = Unmanaged<
          DualParamClosureBox<
            UnsafeMutablePointer<GtkWidget>?,
            OpaquePointer,
            ()
